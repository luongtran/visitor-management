class DashboardController < ApplicationController
  ...
  def get_visitors(start, end_t, page)
    ...
    @most_visitors = Visitor.select('visitor_name, visitor_mobile_number, count(visitor_name) as num_visitors').where('user_id = ?', current_user.id).group('visitor_name').order('num_visitors desc').limit(5) 
